...
void UI::runUserMode()
{
    while (true)
    {
        ...
        if (command == "show")
        {
            openCSV(this->ctrl.getWatchList());
...
void UI::watchStart()
{
    ...
    while(!ls.isEmpty())
    {
        ...
        if (command == "add")
        {
            this->ctrl.addMovieToWatchList("movie.csv", m);
            ls.deleteMovie(m);
        }
        if (command == "next")
            ls.next();
...
void UI::printWatchList(WatchList ls)
{
    ...
    {
        m = ls.getCurrentMovie();
        std::cout << "Movie with title: " << m->getTitle() << " genre: " << m->getGenre() << " from year: " << m->getYear() << " which has: " << m->getLikes() << " likes and has the trailer:\n " << m->getTrailer() << "\n";
        cnt++;
        if (cnt == ls.getSize())
            break;
        ls.next();
...
{
    std::vector<Movie*> ls = this->ctrl.getAllMovies();
    for (auto m : ls)
    {
        std::cout << "Movie with title: " << m->getTitle() << " genre: " << m->getGenre() << " from year: " << m->getYear() << " which has: " << m->getLikes() << " likes and has the trailer: \n" << m->getTrailer() << "\n";
    }
